...
def geodesic_distance_transform(m):
    ...
    while (~visit_mask).sum() > 0:
        # Add correction for artifacts in the borders
        aux = np.asarray(cc) - connectivity
        aux[aux < 0] = 0
        aux[aux >= np.max(m.shape)] = np.max(m.shape) - 1
        # print(aux)
        neighbors = [tuple(e) for e in aux if not visit_mask[tuple(e)]]
        tentative_distance = [distance_increments[i] for i, e in enumerate(aux)
